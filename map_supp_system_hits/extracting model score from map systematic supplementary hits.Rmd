---
title: "Extracting model interface score - map systematic secondary hits"
...
theme_set(theme_prism(base_size = 10, axis_text_angle = 45) + theme(legend.title = element_text()))
```
...
model_rankings <- list.files(path = here::here("af2complex_out/"), pattern = "ranking_all", recursive = T) %>% 
  as_tibble() %>% 
  filter(str_detect(value, "system")) %>% 
...
p2 <- ggplot(model_score_highest, aes(x=fct_reorder(complex_no_map, score, .desc = T), y = score)) +
  ...
  coord_flip()
p1+p2
#ggsave(here::here("AF2Complex results with map systematic secondary hits.svg"), width = 9, height = 10)
```
